...
def generate_video(prompt):
    ...
    try:
        ...
        image_pattern = os.path.join(OUTPUT_DIR, "generated_image.png")  # Use the single generated image
        video_path = os.path.join(OUTPUT_DIR, "generated_video.mp4")

        ffmpeg.input(image_pattern, framerate=1).output(video_path, audio=audio_path).run(overwrite_output=True)
